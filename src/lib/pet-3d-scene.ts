...
export function createPetScene(canvas: HTMLCanvasElement, species: 'cat' | 'dog', gameMode: boolean) {
  ...
  scene.add(new THREE.HemisphereLight(0xfff9ef, 0xb4a6a0, 2.4));
  const light = new THREE.DirectionalLight(0xffebd2, 3.2); light.position.set(-3, 7, 5); light.castShadow = true;
  ...
  const rim = new THREE.DirectionalLight(0xcddfff, 2); rim.position.set(4, 3, -3); scene.add(rim);
  ...
  ellipsoid(torso, fur, [0, 1.0, -.13], [.6, .64, .83]);
  ellipsoid(torso, cream, [0, 1.06, .53], [.43, .48, .15]);
  const head = new THREE.Group(); head.position.set(0, 1.72, .56); torso.add(head);
  ellipsoid(head, fur, [0, 0, 0], [.7, .64, .59]);
  ellipsoid(head, cream, [0, -.25, .47], [.43, .24, .22]);
  ellipsoid(head, pink, [0, -.17, .68], [.105, .065, .065]);
  const eyes = [-1, 1].map(side => {
    const eye = ellipsoid(head, dark, [side * .265, .04, .54], [.085, .115, .045]);
    ellipsoid(eye, white, [.23, .3, .78], [.25, .25, .2]);
    ellipsoid(head, pink, [side * .43, -.19, .48], [.12, .055, .035]); return eye;
  });
  for (const side of [-1, 1]) {
    if (species === 'cat') {
      const ear = new THREE.Mesh(geometry(new THREE.ConeGeometry(.24, .6, 3)), fur); ear.position.set(side * .47, .56, -.02); ear.rotation.z = side * -.15; head.add(ear);
      ...
      const ear = ellipsoid(head, brown, [side * .65, -.09, -.03], [.22, .57, .25]); ear.rotation.z = side * .18;
    }
  }
  const mouth = new THREE.Mesh(geometry(new THREE.TorusGeometry(.12, .012, 5, 16, Math.PI)), brown); mouth.rotation.z = Math.PI; mouth.position.set(0, -.3, .66); head.add(mouth);
  const band = new THREE.Mesh(geometry(new THREE.TorusGeometry(.37, .06, 8, 28)), collar); band.rotation.x = Math.PI / 2; band.position.set(0, 1.37, .46); torso.add(band);
  ellipsoid(torso, gold, [0, 1.27, .8], [.095, .105, .025]);
  const legs: THREE.Group[] = [];
  for (const z of [.43, -.58]) for (const x of [-.39, .39]) {
    const leg = new THREE.Group(); leg.position.set(x, .75, z); rig.add(leg);
    ellipsoid(leg, fur, [0, -.25, 0], [.19, .35, .21]); ellipsoid(leg, cream, [0, -.56, .07], [.22, .16, .3]); legs.push(leg);
  }
  ...
  function update(state: PetRenderState, time: number, dt: number, reduced: boolean) {
    ...
    rig.position.y = smooth(rig.position.y, sleep ? -.58 : roll ? .08 : sitting ? -.19 : jump ? Math.abs(Math.sin(t * 4)) * .65 : running ? Math.abs(Math.sin(phase)) * .065 : 0, dt);
    ...
    rig.rotation.x = smooth(rig.rotation.x, sleep ? -.18 : 0, dt);
    ...
    legs.forEach((leg, i) => {
      let angle = running ? Math.sin(phase + (i === 0 || i === 3 ? 0 : Math.PI)) * .65 : sleep ? -1.2 : sitting && i >= 2 ? -1.1 : 0;
  ...
  function resize(width: number, height: number) {
    ...
    const aspect = width / Math.max(1, height), halfWidth = gameMode ? 4.8 : 2.1;
  ...
  function dispose() { geometries.forEach(item => item.dispose()); materials.forEach(item => item.dispose()); light.shadow.map?.dispose(); renderer.dispose(); renderer.forceContextLoss(); }
