...
export type FamilyPet={id:string;name:string;species:'cat'|'dog'};
...
export function PetFamily({pets,spaceId,zh}:{pets:FamilyPet[];spaceId:string;zh:boolean}) {
  ...
  return <section className="pet-family" aria-label={zh?'伙伴们的活动':'Friends together'}><div className={`pet-family-yard ${active?`is-${mode}`:''}`}>
    ...
    {pets.map((pet,index)=><div className="pet-family-friend" key={pet.id} style={{left:`${(index+.5)/pets.length*100}%`,animationDelay:`${-index*1.3}s`}}><PetPortrait species={pet.species} mood={mood} heading={index%2?-.55:.55}/><span>{pet.name}</span></div>)}
