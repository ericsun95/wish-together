...
import type { Coordinates } from "@/lib/wish-drafts";
...
import './task-map.css';
const WishMapCanvas = dynamic(() => import("./wish-map-canvas").then(m => m.WishMapCanvas), { ssr: false, loading: () => <div className="tm-map-empty">…</div> });
export type MapTask = {location?:Coordinates|null;id:string;title:string;address:string;url:string;category:string;status:'wanted'|'planned'|'done';plannedDate:string};
...
export function TaskMap({wishes,zh,onDetails,onEdit}:{wishes:MapTask[];zh:boolean;onDetails?:(id:string)=>void;onEdit?:(id:string)=>void}) {
  ...
  const external=destination?getGoogleMapsUrl(destination):getGoogleMapsUrl(active?.location?`${active.location.latitude},${active.location.longitude}`:source,active?.title);
  ...
  return <section className="task-map" aria-label={zh?'任务地图':'Task map'}>
    ...
    <div className="tm-shell"><aside className="tm-sidebar" aria-label={zh?'地点列表':'Places'}>
      ...
      <ul className="tm-places">{filtered.map(wish=><li key={wish.id}><button type="button" className="tm-place" aria-pressed={!destination&&active?.id===wish.id} onClick={()=>choose(wish.id)}><span className={`tm-place-icon status-${wish.status}`}>{wish.status==='done'?<Check size={17}/>:<MapPin size={17}/>}</span><span className="tm-place-copy"><span className="tm-place-title">{wish.title}</span><span className="tm-address">{address(wish)}</span>{!wish.location&&<span className="tm-address">{zh?"待定位 · 编辑并搜索地点":"Unpinned · edit to select a place"}</span>}<span className="tm-meta"><span className={`tm-badge status-${wish.status}`}>{statuses[wish.status]}</span>{wish.category&&<span>{wish.category}</span>}{wish.plannedDate&&<span><CalendarDays size={11}/>{wish.plannedDate}</span>}</span></span><ChevronRight size={14} className="tm-chevron"/></button></li>)}</ul>
      ...
      {!destination&&<WishMapCanvas wishes={filtered} selected={selected} onSelect={choose} zh={zh}/>}
      {query?<>{destination&&<MapEmbed key={`${query}:${zh}`} query={query} title={`${zh?'任务地点地图':'Task location map'}: ${destination}`} zh={zh} external={external}/>}<div className="tm-detail"><div className="tm-detail-text"><span className="tm-eyebrow">{destination?(zh?'地点搜索':'SEARCH RESULT'):(active?.category||(zh?'已保存地点':'SAVED PLACE'))}</span><h2>{destination||active?.title}</h2><p><MapPin size={14}/>{destination||(active?address(active):'')}</p></div><div className="tm-detail-actions">{!destination&&active&&onEdit&&<button type="button" onClick={()=>onEdit(active.id)}>{zh?"编辑心愿 / 定位":"Edit wish / pin"}</button>}{!destination&&active&&onDetails&&<button type="button" onClick={()=>onDetails(active.id)}>{zh?'任务详情':'Task details'}<ChevronRight size={14}/></button>}<a href={external} target="_blank" rel="noopener noreferrer"><Navigation size={15}/>{zh?'打开地图':'Open Maps'}<ArrowUpRight size={14}/></a></div></div><p className="tm-provider">{zh?'搜索选点的心愿显示精确标记；没有坐标的旧地址可通过编辑补充定位。':'Selected search results have precise pins. Edit older addresses to add their location.'}</p></>:<div className="tm-map-empty"><div><MapPin size={30}/></div><h2>{zh?'下一站，想去哪里？':'Where to next?'}</h2><p>{zh?'选择一个任务地点，或搜索你想去的地方。':'Select a saved place or search for somewhere new.'}</p></div>}
    </div></div>
  </section>;
}
