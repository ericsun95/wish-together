...
type Pet = { id:string; slot:number; space_id: string; name: string; species: Species; appearance: string; experience: number; created_at: string };
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [petResult, careResult, memberResult, authResult] = await Promise.all([
        supabase.from('space_pets').select('id,slot,space_id,name,species,appearance,experience,created_at').eq('space_id', spaceId).order('slot'),
  ...
  async function saveName(event: FormEvent) {
    ...
    try {
      const result = renaming && pet
        ...
        : await supabase.from('space_pets').insert({ space_id: spaceId, name: name.trim(), species, appearance, created_by: userId }).select('id').single();
    ...
    {!loaded ? <p className="life-empty">{error ? (zh ? '点击刷新，再来看看它。' : 'Refresh to try again.') : (zh ? '正在布置宠物的小家…' : 'Getting your pet’s home ready…')}</p> : !pet || adopting ?
      <form className="pet-adoption" onSubmit={saveName}>
        ...
        <div className="pet-adoption-form"><h3>{zh ? (pets.length?'给它找个新伙伴':'领养你们的第一只宠物') : 'Welcome a new friend'}</h3><p className="life-muted">{zh ? '一起取名字、喂食、陪玩，把每天的小小陪伴，变成它的成长。' : 'Name, feed and play with your pet. Your everyday care helps it grow.'}</p>
          ...
          <p className="life-muted">{zh ? '小窝最多住 8 只，每只都有自己的名字和成长记录。' : 'Up to 8 pets, each with their own name and growth.'}</p>
