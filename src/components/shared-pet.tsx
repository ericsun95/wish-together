  ...
  return <section className="life-section pet-home" aria-label={zh ? '我们的宠物' : 'Our pet'}>
    ...
    {!loaded ? <p className="life-empty">{error ? (zh ? '点击刷新，再来看看它。' : 'Refresh to try again.') : (zh ? '正在布置宠物的小家…' : 'Getting your pet’s home ready…')}</p> : !pet ?
      <form className="pet-adoption" onSubmit={saveName}>
        ...
        <div className="pet-card"><div className="pet-scene"><span className="pet-level">Lv. {level}</span><PetCarry spaceId={spaceId} zh={zh}><PetPortrait species={pet.species} happy={!!notice} mood={pose}/></PetCarry><div className="pet-pose-picker" role="group" aria-label={zh?"宠物姿态":"Pet poses"}>{PET_POSES.map(item=><button type="button" key={item.mood} aria-pressed={pose===item.mood} onClick={()=>{setNotice('');setPose(item.mood);}}>{item.emoji} {zh?item.zh:item.en}</button>)}</div><p>{zh ? '有你们在，每天都很开心' : 'Every day is happier with you two'}</p></div>
